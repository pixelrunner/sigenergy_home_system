...
# Touch Exit Handler (Hold screen for 1 second to return to launcher menu)
is_holding = False
touch_press_start = 0
...
def check_touch_exit():
    ...
    if presto.touch.state:
        ...
        if not is_holding:
            is_holding = True
            touch_press_start = now_ms
        elif time.ticks_diff(now_ms, touch_press_start) >= 1000:
            # Force immediate hardware reboot back to main.py
            machine.reset()
    else:
        is_holding = False
...
while True:
    # 1. Poll touch hardware continuously
    check_touch_exit()

    # 2. Fetch telemetry non-blockingly every 5 seconds
    ...
    # Crucial yield delay to keep RP2350 responsive
    time.sleep(0.05)
